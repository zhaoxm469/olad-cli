...
import { replaceDockerVersion } from "./util.js";
import { PROJECT_PATH, QA_PROJECT_REPO_YML_MAP } from "./configs.js";

export async function modifyEnvRepoYmlConfig (projectName,dockerImgVersion,envVar){
  const ymlFileNameMapping = QA_PROJECT_REPO_YML_MAP[projectName] || projectName
  const getGitRepoPath = envVar=> `${PROJECT_PATH}cluster-${envVar}/`
  const ymlConfigPath = `${getGitRepoPath(envVar)}deploy/release/${ymlFileNameMapping}.yml`
  ...
  // 替换docker版本号
  const newYmlContent = replaceDockerVersion(ymlConfigContent,dockerImgVersion)
